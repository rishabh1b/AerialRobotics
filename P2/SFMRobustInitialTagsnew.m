...
%     end
    ...
    if ~skipped
        ...
        graph.add(BetweenFactorPose3(poses{i-2}, poses{i}, Pose3, constraintNoise2));
        skipped  = false;
    end
    % Change the tag_ids
...
for i = 1 : num_poses
    ...
    %rottemp = result.at(poses{i}).rotation.matrix;
    %AllPosesComputed(i,:) = (-rottemp' * AllPosesComputed(i,:)')';
end
%% Plot
t = size(pointsComputed,1);
index = 1;
while index <= t
    scatter3(pointsComputed(index,1),pointsComputed(index,2),0, 'ro')
    hold on
    scatter3(pointsComputed(index+1,1),pointsComputed(index+1,2),0, 'go')
    scatter3(pointsComputed(index+2,1),pointsComputed(index+2,2),0, 'bo')
    scatter3(pointsComputed(index+3,1),pointsComputed(index+3,2),0, 'mo')
    index = index + 4;
end
%% Plot Poses
hold on
...
%% Plot Ground Truth
%scatter3(gtposes(:,11),gtposes(:,12),gtposes(:,13), 'ko')